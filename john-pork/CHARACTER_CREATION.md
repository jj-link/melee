...
The combined Stephen Hawking/John Pork game uses the repository-root `play-custom-smash.cmd`. That launcher opens the repository-root `Melee - Custom Smash.iso` with its separate profile under `output/custom-smash/DolphinUser`. The builder moves an existing Hawking-local profile there only if no root-level profile exists. Dolphin is discovered from an explicit launcher argument, `DOLPHIN_EXE`, the remembered selection, `PATH`, or conventional Windows installation folders; otherwise a file picker asks for `Dolphin.exe` and remembers it locally.

The normal combined build, `stephen-hawking/tools/build_fighter.py`, creates a missing `stephen-hawking/prototype/stephen-hawking.blend` from the checked-in models and textures without rendering previews. It preserves an existing prototype and regenerates the runtime exports as usual. A new checkout does not need Blender files copied from another computer or a separate John-Pork-only build.
...
- The combined build was run on Windows with all four existing Blender files absent from their normal locations. It generated the required Hawking files automatically, wrote the game at the repository root, and migrated the Dolphin profile with its configuration files byte-identical. The root launcher was exercised from another working directory: selecting Dolphin once launched the game, and a second launch reused the selection without prompting. The rebuilt game ran Hawking versus Kirby on Final Destination. Missing-game guidance and rejection of the generated ISO as the clean source were also checked. This was not a test on a second physical computer.
